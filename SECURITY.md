...
- **Public-spend protection — a revocable access capability, not an OpenRouter credential:** a second, unrelated server secret, `JON_SNOW_DEMO_ACCESS_TOKEN`, gates who may invoke the endpoint at all, checked (constant-time comparison, `netlify/server/tribunal/demoAccess.ts`) against the request's `X-Jon-Snow-Demo-Access` header before any case/run/provider work happens. Missing or invalid: zero case/run creation, zero provider execution, response indistinguishable from "server not configured." The lecturer never sees or types this token — it is carried in a prepared presentation link's URL **fragment** (never sent to any server automatically), captured once client-side into `sessionStorage`, and immediately stripped from the visible URL/history (`src/services/jonSnowDemoAccess.ts`).
