-- Milestone 7 — Prompt-version bridge.
--
-- This migration is NOT applied to the real development database as part
-- of implementation (Section 30 of the Milestone 7 implementation task).
-- It requires an independent static audit before it becomes historical
-- remote state, matching the discipline already established for the
-- Milestone 5/6 migrations.
--
-- Design contract: docs/adr/0003-openrouter-infrastructure.md
-- Decisions 16, 17. The already-applied Milestone 6 migration
-- (20260825214212_participant_configuration.sql) is NEVER edited by this
-- file -- this is a brand-new forward migration that CREATE OR REPLACEs
-- the freeze function with the identical signature, SECURITY DEFINER
...
-- grants, and returned columns/behavior. The ONLY intended behavioral
-- change: the hardcoded prompt_version literal written per new row is now
-- role-specific ('advocate-v1' / 'judge-v1', src/prompts/versions.ts's
-- ADVOCATE_PROMPT_VERSION / JUDGE_PROMPT_VERSION) instead of the M6
-- placeholder ('unassigned-pre-m7'). Every other line is copied verbatim
-- from the M6 function body.
--
-- No historical participant_configs row is mutated by this migration --
-- CREATE OR REPLACE FUNCTION only replaces the function definition itself,
-- never touches existing table data. Every M6 run frozen with
-- 'unassigned-pre-m7' remains exactly as it was and stays permanently
-- execution-ineligible (SPEC.md MODEL-006) -- there is no UPDATE
...
-- function still only ever writes 'READY' on a new run (M7's standalone
-- preflight never touches POST /api/runs or this function at all, per ADR
-- Decision 14).
...
declare
  ...
  v_role text;
  v_side text;
  -- NEW in this migration: role-specific prompt_version, derived
  -- internally alongside v_role/v_side -- never a caller parameter.
...
begin
  ...
  if v_new_run_id is not null then
    ...
    loop
      ...
      -- NEW in this migration: role-specific prompt_version, matching the
      -- exact literal values src/prompts/versions.ts exports
      -- (ADVOCATE_PROMPT_VERSION / JUDGE_PROMPT_VERSION). Never a caller
      -- parameter -- derived from the same fixed v_role mapping above.
      v_prompt_version := case
        when v_role = 'ADVOCATE' then 'advocate-v1'
        else 'judge-v1'
      end;
...
-- does not silently depend on the M6 migration having already run
-- correctly.
