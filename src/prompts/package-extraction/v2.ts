// Milestone 7A / PRO-CON semantic correction (Issue #30) --
// package-extraction-v2's system prompt. package-extraction-v1
// (v1.ts) never defined what the PRO_*/CON_* seat keys mean, which
// risked a free-form dossier's "Prosecution" representative being
// (mis)mapped to a PRO_* seat purely from surface lexical similarity to
// the letters "pro". v2 adds one explicit sentence defining the
// semantic mapping -- PRO_* = the defendant's Defense (supports the
// defendant, argues toward NOT_GUILTY); CON_* = the
// Opposition/Prosecution (argues against the defendant, toward GUILTY)
// -- and explicitly warns against inferring the mapping from surface
// wording alone.
//
// This module is IMMUTABLE once package-extraction-v2 has been used by
// any accepted logical extraction -- a materially different prompt is a
// NEW version (v3.ts, additive), never an in-place edit here. Resolved
// only through registry.ts; no other module imports this file directly.
// package-extraction-v1 (v1.ts) is never edited by this file and remains
// permanently resolvable for historical extraction replay/audit (ADR
// 0004 Decision 15's frozen-identity rule).

export function buildPackageExtractionSystemPromptV2(): string {
  return [
    ...
    "The PRO_* and CON_* seat keys have a fixed real-world meaning: PRO_1/PRO_2 are the defendant's Defense representatives -- they support the defendant and argue toward a NOT_GUILTY outcome. CON_1/CON_2 are the Opposition/Prosecution representatives -- they argue against the defendant, toward a GUILTY outcome. Map dossier content to these seats by its stated real-world role (defense/supports-the-defendant vs. prosecution/opposition/against-the-defendant), never by surface wording similarity to the letters \"pro\" or \"con\" -- a participant explicitly described as \"the prosecution\" or \"prosecutor\" belongs in a CON_* seat, not a PRO_* seat, regardless of that surface resemblance.",
...
// extraction token-estimation path, which is computed FROM this exact
// text -- never guessed.
export const PACKAGE_EXTRACTION_SYSTEM_PROMPT_V2 = buildPackageExtractionSystemPromptV2();
