...
- **Featured: Jon Snow Demo** — a themed card that is a single entry point: the whole card links to `/demo/jon-snow`, labeled **Modify settings**. It does not itself submit or execute a run.
...
**Home's Jon Snow card** is an entry point only, not a run trigger: it shows truthful, live context — the configured default model's name, the demo's own operator-funded ceiling, and an access-dependent status line ("Ready to review the canonical demo settings." when a lecturer access capability is present in `sessionStorage`, "Lecturer access is required to run; settings remain reviewable." when it is not, or a review prompt when the default model is currently outside the ceiling) — sourced entirely from the existing eligible-model metadata endpoint, never computed in browser code. Clicking anywhere on the card navigates to `/demo/jon-snow`; no case or run is ever submitted from Home itself.

**`/demo/jon-snow` ("Modify settings / models")** is the dedicated Jon Snow demo/settings experience, presented in its own dark cinematic style distinct from the rest of the product: the full canonical case, all seven seats (advocates and judges) with their real canonical personalities rendered in full, the dossier's own global disclaimer, and a model chooser restricted to models that are both currently eligible and within the demo's cost ceiling — an over-ceiling model is omitted from the list entirely, never shown disabled. It carries no OpenRouter credential field; the demo is operator-funded, so there is nothing for the lecturer to connect here. The primary **Run Jon Snow Demo** action lives on this page, not on Home, and additionally requires the same lecturer access capability described above — without it, the action stays disabled with a concise explanation directing the lecturer to open the prepared demo link; there is no silent fallback to a different or costlier model, and running is not possible from anywhere else. Running here uses the same dedicated demo endpoint and shared idempotent-submission primitive as every other Convene path in this document.

On a successful run start, execution navigates to the exact same generic **`/runs/:runId`** every other Tribunal run uses — there is no dedicated Jon Snow active-run or completed-result presentation. The running and completed states render through the same Ivory & Iron / white-gold RunPage and result experience described in §11/§14, completely unmodified: the Jon Snow identity already lives in the case/run data itself (defendant, canonical case, participant names), not in a page-level presentation wrapper. The legacy `/demo/jon-snow/runs/:runId` URL is preserved only for backward compatibility and immediately redirects to `/runs/:runId`, carrying the same run id, with no data fetching or execution of its own. Game-of-Thrones theming (the app-wide dark-chamber theme swap) is confined to the exact `/demo/jon-snow` settings route — never the run/result screen, regardless of which route was used to reach it or whose run it is; Home's own card keeps its own component-level dark accent as a promotional treatment, independent of that route-level theme swap.
